...
@contextmanager
def get_connection(operations) -> Generator[sqlalchemy.engine.Connection, None, None]:
    """
...
@alembic.autogenerate.comparators.dispatch_for("schema")  # type: ignore
        ...
        for name, new_values in declared.enum_definitions.items():
            ...
            if name in defined.enum_definitions and new_values != old_values:
                affected_columns = frozenset(
                    ...
                    for table_definition in (declared.table_definitions or [])
